...
public class SuperAdminTraderController {
    ...
    public ResponseEntity<TraderResponse> changeStatus(@PathVariable Long id, @RequestBody @Valid ChangeTraderStatusRequest request) {
        return ResponseEntity.ok(traderManagementService.changeStatus(id, request.status()));
    }

    @PutMapping("/{id}")
    public ResponseEntity<TraderResponse> updateTrader(@PathVariable Long id, @Valid @RequestBody UpdateTraderRequest request) {
